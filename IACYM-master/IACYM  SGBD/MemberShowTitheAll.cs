...
{
    public partial class MemberShowTitheAll : Form
    {
        private int Code;
        public MemberShowTitheAll(int codigo)
        {
            InitializeComponent();
            this.Code = codigo;
        ...
        {
            SqlConnection connection = new SqlConnection(General_Values.str_connection);
            string str_consult = "SELECT * FROM diezmo WHERE cod_miembro = " + this.Code + ";";
        ...
        private void ButtonBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (TextBoxBuscarMes.Text != "Ingrese Mes" && TextBoxBuscarAño.Text == "Ingrese Año")
            {
                ...
                if (check)
                {
                    SqlConnection connection = new SqlConnection(General_Values.str_connection);
                    connection.Open();
                    string str_consult = "SELECT * FROM diezmo WHERE mes = '" + mes + "' AND cod_miembro = " + this.Code + ";";
                    SqlCommand consult = new SqlCommand(str_consult, connection);
                    SqlDataReader record = consult.ExecuteReader();
            ...
            else if (TextBoxBuscarMes.Text == "Ingrese Mes" && TextBoxBuscarAño.Text != "Ingrese Año")
            {
                ...
                if (check)
                {
                    SqlConnection connection = new SqlConnection(General_Values.str_connection);
                    connection.Open();
                    string str_consult = "SELECT * FROM diezmo WHERE año = '" + año + "' AND cod_miembro = " + this.Code + ";";
        ...
        private void ButtonEliminar_Click(object sender, EventArgs e)
        {
            ...
            else if (!Id.ExistsId("diezmo", "id", id))
                check = false;

            if (check)
            {
                Id.DeleteId("diezmo", "id", int.Parse(id));
