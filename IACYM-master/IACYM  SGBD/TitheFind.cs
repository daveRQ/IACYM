...
{
    public partial class TitheFind : Form
    {
        public TitheFind()
        {
            InitializeComponent();
        ...
        private void ButtonBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (TextBoxMes.Text != "Ingrese Mes" && TextBoxAño.Text == "Ingrese Año")
            {
                ...
                if (check)
                {
                    ...
                    string str_consult = "SELECT P.cod_miembro, M.nombre, M.ap_paterno, M.ap_materno, P.año, P.mes, P.monto, P.fch_emision " +
                                        "FROM diezmo P JOIN miembro M ON P.cod_miembro = M.codigo WHERE P.mes = '" + mes + "';";
                    SqlCommand consult = new SqlCommand(str_consult, connection);
                    SqlDataReader record = consult.ExecuteReader();
                    DataGridViewDiezmo.Rows.Clear();
                    while (record.Read())
                    {
                        int n = DataGridViewDiezmo.Rows.Add();
                        DataGridViewDiezmo.Rows[n].Cells[0].Value = record["cod_miembro"];
                        DataGridViewDiezmo.Rows[n].Cells[1].Value = record["nombre"];
                        DataGridViewDiezmo.Rows[n].Cells[2].Value = record["ap_paterno"].ToString() + " " + record["ap_materno"].ToString();
                        DataGridViewDiezmo.Rows[n].Cells[3].Value = record["año"];
                        DataGridViewDiezmo.Rows[n].Cells[4].Value = record["mes"];
                        DataGridViewDiezmo.Rows[n].Cells[5].Value = record["monto"];

                        DateTime temp = DateTime.Parse(record["fch_emision"].ToString());
                        DataGridViewDiezmo.Rows[n].Cells[6].Value = temp.Day.ToString() + "/" + temp.Month.ToString() + "/" + temp.Year.ToString();
                    }
                    connection.Close();
                }
            }
            ...
            else if (TextBoxMes.Text == "Ingrese Mes" && TextBoxAño.Text != "Ingrese Año")
            {
                ...
                if (check)
                {
                    ...
                    connection.Open();
                    string str_consult = "SELECT P.cod_miembro, M.nombre, M.ap_paterno, M.ap_materno, P.año, P.mes, P.monto, P.fch_emision " +
                                        "FROM diezmo P JOIN miembro M ON P.cod_miembro = M.codigo WHERE P.año = '" + año + "';";
                    SqlCommand consult = new SqlCommand(str_consult, connection);
                    SqlDataReader record = consult.ExecuteReader();
                    DataGridViewDiezmo.Rows.Clear();
                    while (record.Read())
            ...
            else if (TextBoxMes.Text != "Ingrese Mes" && TextBoxAño.Text != "Ingrese Año")
            {
                ...
                if (check)
                {
                    ...
                    connection.Open();
                    string str_consult = "SELECT P.cod_miembro, M.nombre, M.ap_paterno, M.ap_materno, P.año, P.mes, P.monto, P.fch_emision " +
                                        "FROM diezmo P JOIN miembro M ON P.cod_miembro = M.codigo WHERE P.año = '" + año + "' AND P.mes = '" + mes + "';";
                    SqlCommand consult = new SqlCommand(str_consult, connection);
                    SqlDataReader record = consult.ExecuteReader();
                    DataGridViewDiezmo.Rows.Clear();
                    while (record.Read())
        ...
        private void ButtonMostrarTodo_Click(object sender, EventArgs e)
        {
            ...
            connection.Open();
            string str_consult = "SELECT P.cod_miembro, M.nombre, M.ap_paterno, M.ap_materno, P.año, P.mes, P.monto, P.fch_emision " +
                                "FROM diezmo P JOIN miembro M ON P.cod_miembro = M.codigo;";
            SqlCommand consult = new SqlCommand(str_consult, connection);
            SqlDataReader record = consult.ExecuteReader();
            DataGridViewDiezmo.Rows.Clear();
            while (record.Read())
        ...
        {
            decimal total = 0;
            for (int i = 0; i < DataGridViewDiezmo.Rows.Count - 1; ++i)
                total += decimal.Parse(DataGridViewDiezmo.Rows[i].Cells[5].Value.ToString());
